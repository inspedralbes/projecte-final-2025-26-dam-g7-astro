...
export const useAstroStore = defineStore('astro', () => {
  ...
  async function loginTripulante (credentials) {
    ...
    if (Array.isArray(profile.friendRequests)) {
      socialStore.setFriendRequests(profile.friendRequests)
    }
  ...
  async function sendFriendRequest (friendName) {
    return socialStore.sendFriendRequest(friendName)
  }
  async function acceptFriendRequest (friendName) {
    return socialStore.acceptFriendRequest(friendName)
  }
  async function rejectFriendRequest (friendName) {
    return socialStore.rejectFriendRequest(friendName)
  }
