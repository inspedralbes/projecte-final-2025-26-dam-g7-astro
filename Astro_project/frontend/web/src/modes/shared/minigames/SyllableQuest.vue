  ...
  <v-container class="fill-height d-flex flex-column align-center justify-center syllable-quest-container pa-4" style="position: relative;" @mousemove="handleMouseMove" @click="handlePlayAreaClick" :class="{ 'game-paused': props.isPaused, 'freeze-cursor-hide': multiplayerStore.activeBossEffect === 'FREEZE' }" fluid>
    <v-card class="pa-6 bg-slate-900 border-amber game-shell mb-10 game-board" max-width="560" rounded="xl" width="100%">
      ...
      <template v-if="!gameFinished">
        <div v-if="!isMultiplayer || props.isDuel || props.isRace || (multiplayerStore.room?.gameConfig?.mode === 'TOURNAMENT') || (multiplayerStore.room?.gameConfig?.modality === '1vs1')">
          ...
          <!-- Indicador visual de palmadas de sílabas -->
          <div class="d-flex justify-center ga-1 mb-4">
            <v-icon
              v-for="i in 8"
              :key="'syllable-dot-'+i"
              :color="i <= userSyllables ? 'amber-accent-3' : 'grey-darken-3'"
              :icon="i <= userSyllables ? 'mdi-hand-clap' : 'mdi-circle-medium'"
              size="24"
            />
          </div>
...
function checkSyllables() {
  ...
  if (userSyllables.value === currentWord.value.syllables) {
    ...
    if (props.isMultiplayer) {
      ...
      if (props.isDuel || props.isRace || multiplayerStore.room?.gameConfig?.modality === '1vs1' || multiplayerStore.room?.gameConfig?.mode === 'TOURNAMENT') {
    ...
    userSyllables.value = 0 // Restablecer a 0 tras error para permitir nuevo intento
...
function processCorrectClick(idx, word) {
  ...
  score.value += 15
  timeLeft.value += 3 // +3s por acierto de palabra en orden
  successfulDetections.value++
  triggerFeedback('success')
