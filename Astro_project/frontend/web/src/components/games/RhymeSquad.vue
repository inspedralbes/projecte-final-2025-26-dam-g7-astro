<template>
  <v-container class="d-flex flex-column align-center justify-start game-container pa-0"
    :class="{ 'game-paused': props.isPaused }" fluid style="height: 100vh; overflow: hidden;">

    <v-card v-if="!isPlaying && !isGameOver" class="pa-10 text-center bg-grey-darken-4 border-cyan" max-width="800"
      rounded="xl" width="100%">
      ...
      <div v-if="isMultiplayer" class="mb-8">
        <v-row>
          <v-col cols="12" md="6">
            <div
              ...
              <v-icon :color="subRole === 'catcher' ? 'green-accent-3' : 'grey'" size="64"
                class="mb-4">mdi-basket-fill</v-icon>
              <h2 class="text-h4 font-weight-black mb-2"
                :class="subRole === 'catcher' ? 'text-green-accent-3' : 'text-grey'">{{ $t('rhymeSquad.roleCatcher') ||
                'RECOL·LECTOR' }}</h2>
              <p class="text-body-1 text-white">{{ $t('rhymeSquad.catcherDesc') || '¡Atrapa només les paraules que rimin
                amb la pista!' }}</p>
          ...
          <v-col cols="12" md="6">
            <div
              ...
              <h2 class="text-h4 font-weight-black mb-2"
                :class="subRole === 'sniper' ? 'text-red-accent-3' : 'text-grey'">{{ $t('rhymeSquad.roleSniper') ||
                'DESTRUCTOR' }}</h2>
              <p class="text-body-1 text-white">{{ $t('rhymeSquad.sniperDesc') || '¡Destrueix les paraules que NO rimin
                per protegir al company!' }}</p>
        ...
        <div v-if="!isHost" class="mt-4 text-center">
          ...
          <p class="text-h6 text-cyan-accent-3 mt-4">{{ $t('rhymeSquad.waitingForHost') || 'Esperant que el Comandant
            iniciï la missió...' }}</p>
        </div>
      </div>

      <v-btn v-if="!isMultiplayer" block class="font-weight-black text-black text-h6" color="cyan-accent-3" height="60"
        rounded="pill" size="x-large" @click="handleStartClick">
      ...
      <div class="arcade-hud w-100 pa-4 d-flex justify-space-between align-center">
        ...
        <div class="hud-right d-flex gap-4 align-center">
          ...
          <div class="lives-display d-flex gap-1">
            <v-icon v-for="i in (props.isMultiplayer ? 10 : 3)" :key="i"
              :icon="i <= lives ? 'mdi-heart' : 'mdi-heart-outline'"
              :color="i <= lives ? 'red-accent-2' : 'grey-darken-2'" size="24" class="heart-icon" />
          </div>
        </div>
      </div>

      <div class="play-area position-relative w-100" :class="{ 'turbo-mode': isTurbo }"
        ...
        <transition-group name="fade">
          <div v-for="word in activeWords" :key="word.id" class="falling-word" :class="getWordStatusClass(word.status)"
            :style="{ left: word.x + '%', animationDuration: word.speed + 's' }"
            @animationend="removeWord(word.id, false)" @mousedown="catchWord(word)">
            {{ word.text }}
          </div>
        </transition-group>
        ...
        <v-overlay v-model="showFeedback" contained class="align-center justify-center pointer-events-none" persistent
          no-click-animation scrim="transparent">
    ...
    <v-card v-else-if="isGameOver" class="pa-10 text-center bg-grey-darken-4 border-cyan" max-width="600" rounded="xl"
      width="100%">
      <v-icon class="mb-4" :color="lives > 0 ? 'cyan-accent-2' : 'red-accent-2'"
        :icon="lives > 0 ? 'mdi-flag-checkered' : 'mdi-skull-crossbones'" size="100" />
      <h2 class="text-h3 text-white mb-2">{{ lives > 0 ? $t('rhymeSquad.timeOut') : $t('rhymeSquad.missionFailed') }}
      </h2>
      ...
      <v-btn color="cyan-accent-3" height="60" size="x-large" rounded="pill" variant="flat"
        class="text-black font-weight-bold text-h6 block w-100" @click="emitExit">
