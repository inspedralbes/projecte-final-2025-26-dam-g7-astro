...
async function initializeUsers() {
    try {
        ...
        // 1. PREGUNTAMOS: ¿Hay alguien ahí?
        const count = await collection.countDocuments();

        if (count === 0) {
            // CASO A: Base de datos virgen. Creamos los usuarios por defecto.
            console.log('⚠️ Base de dades buida. Generant tripulació inicial...');
            await collection.insertMany(initialUsers);
            console.log('✅ Usuarios iniciales creados.');
        } else {
            // CASO B: Ya hay datos. NO TOCAMOS NADA.
            // Aquí es donde respetamos que JOEL quizás ya gastó monedas o ganó premios.
            console.log(`ℹ️ Base de datos cargada: ${count} tripulantes detectados. Respetando sus saldos.`);
        }
    } catch (error) {
